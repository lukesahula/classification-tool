...
\begin{document}
  ...
  \section{Random forest classifier}
    ...
    \subsection{Decision trees}
      ...
      \subsubsection{When to create a leaf node}
        ...
        \begin{itemize}
          ...
          \item One of the parameters of a decision tree is usually the minimum number of samples required to split the dataset ($minSamplesToSplit$). If $|\mathbf{S_i}| < minSamplesToSplit$ then a leaf node is created. This parameter is often set low in random forests because it allows the trees to grow deeper. \cite{brabec}
        \end{itemize}
        Other implementations also include the $maxDepth$ parameter, limiting the maximum depth of a tree, or the change in {\it impurity measure} (information gain) as the stopping condition during node splitting. \cite{brabec}
    ...
    \subsection{Random forests}
      Since single decision forests tend to overfit to the training data\cite{icl}, are not making very accurate predictions, and tend to change significantly with small alterations to the datasets, they do not look very good compared to other approaches. However, {\it ensemble method} called random forests was invented. It puts randomized decision trees together and builds a much stronger classifier.
      In this subsection the process of putting the decision trees together to form a random forest is going to be explained.
